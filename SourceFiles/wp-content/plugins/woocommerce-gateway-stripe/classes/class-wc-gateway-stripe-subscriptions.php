...
class WC_Gateway_Stripe_Subscriptions extends WC_Gateway_Stripe {
	...
	function process_payment( $order_id ) {
		...
		if ( class_exists( 'WC_Subscriptions_Order' ) && WC_Subscriptions_Order::order_contains_subscription( $order_id ) ) {
			...
			try {
				...
				$woocommerce->add_error( __('Error:', 'woocommerce-gateway-stripe') . ' "' . $e->getMessage() . '"' );
				return;
			}
	...
	function process_subscription_payment( $order = '', $amount = 0 ) {
		...
		if ( $amount * 100 < 50 )
			return new WP_Error( 'stripe_error', __( 'Minimum amount is 0.50', 'woocommerce-gateway-stripe' ) );
	...
	function add_customer_to_order( $order, $customer_id = '', $stripe_token = '' ) {
		...
		elseif ( $stripe_token ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$order->add_order_note( sprintf( __('Stripe customer added: %s', 'woocommerce-gateway-stripe' ), $response->id ) );
	...
	function update_failing_payment_method( $original_order, $renewal_order, $subscription_key ) {
		...
		try {
			...
			$customer_response = $this->add_customer_to_order( $original_order, $customer_id, $stripe_token );
			...
			if ( is_wp_error( $customer_response ) ) {
				...
				// Store token
				if ( $stripe_token )
					update_post_meta( $original_order->id, '_stripe_token', $stripe_token );
