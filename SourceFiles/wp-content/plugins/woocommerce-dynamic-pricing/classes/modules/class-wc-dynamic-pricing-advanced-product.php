...
class WC_Dynamic_Pricing_Advanced_Product extends WC_Dynamic_Pricing_Advanced_Base {
	...
	private function get_pricing_rule_sets($cart_item) {
		$pricing_rule_sets = get_post_meta($cart_item['data']->id, '_pricing_rules', true);
		$sets = array();
	...
	private function get_block_adjusted_price($set, $price, $cart_item) {
		$result = false;
			...
			case 'cat' :
				if (isset($collector['args']) && isset($collector['args']['cats']) && is_array($collector['args']['cats'])) {
					foreach ($collector['args']['cats'] as $cat) {
						if (WC_Dynamic_Pricing_Counter::get_category_count($cat)) {
							$quantity += WC_Dynamic_Pricing_Counter::get_category_count($cat);
						}
					}
				}
				break;
