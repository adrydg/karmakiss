...
Copyright (c) 2012, 2013, 2014 Store Apps All rights reserved.
*/
...
if ( is_woocommerce_active() ) {
        ...
        class WC_Smart_Coupons {
            ...
            public function __construct() {
                ...
                add_action( 'woocommerce_email_after_order_table', array( $this, 'generated_coupon_details_after_order_table' ) );
            ...
            function add_generated_coupon_details() {
                global $post;
            ...
            //
            function sc_generated_coupon_data_metabox() {
                global $post;
                if ( !empty( $post->ID ) ) {
            ...
            function get_generated_coupon_data( $order_ids = '', $user_ids = '', $html = false, $header = false, $layout = 'box' ) {
                ...
                if ( empty( $generated_coupon_data ) ) {
                    if ( is_admin() ) {
                        $js = "jQuery('div.sc_empty_records').show();";
                        if ( Smart_Coupons_WC_Compatibility::is_wc_21() ) {
                            wc_enqueue_js( $js );
                        } else {
                    ...
                    echo '<div class="sc_empty_records" style="display: none;">';
                    echo __( 'No records found', 'wc_smart_coupons' );
                    echo '</div>';
                    return;
                }
            ...
            function get_generated_coupon_data_table( $generated_coupon_data = array() ) {
                if ( empty( $generated_coupon_data ) ) return;
                ?>
                                ...
                                <?php
                                    foreach ( $generated_coupon_data as $from => $data ) {
                                        foreach ( $data as $coupon_data ) {
                                            ...
                                            echo '<td>' . Smart_Coupons_WC_Compatibility::wc_price( $coupon_data['amount'] ) . '</td>';
            ...
            function generated_coupon_details_after_order_table( $order ) {
                if ( !empty( $order->id ) ) {
                    $this->get_generated_coupon_data( $order->id, '', true, true );
                }
            }

            function generated_coupon_details_view_order( $order_id ) {
                if ( !empty( $order_id ) ) {
                    $this->get_generated_coupon_data( $order_id, '', true, true );
                }
            }

            function generated_coupon_details_before_my_account() {
            ...
            public function woocommerce_cart_item_price_html( $product_price, $cart_item, $cart_item_key ) {
                ...
                $gift_certificate = Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called;
            ...
            function sc_woocommerce_add_to_cart_validation( $validation, $product_id, $quantity, $variation_id = '', $variations = '', $cart_item_data = array() ) {
                ...
                if( ! isset( $_POST['credit_called'] ) )
                    return $validation;
                            ...
                            $cart_id = Smart_Coupons_WC_Compatibility::global_wc()->cart->generate_cart_id( $product_id, $variation_id, $variations, $cart_item_data );

                            if ( function_exists( 'get_product' ) ) {
                                if ( isset( Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called[$cart_id] ) && empty( Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called[$cart_id] ) ) {
                                    return false;
                                }
                            } else {
            ...
            function apply_coupon_from_url() {
                ...
                if( isset( $coupon_args['coupon-code'] ) && ! empty( $coupon_args['coupon-code'] ) ) {
                    ...
                    Smart_Coupons_WC_Compatibility::global_wc()->cart->add_discount( trim( $coupon_args['coupon-code'] ) );      
            ...
            function load_sc_textdomain() {
                        ...
                        function save_called_credit_details_in_order( $order_id, $posted ) {
                            ...
                            if( function_exists( 'get_product' ) ) {
                                if ( isset( Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called ) ) unset( Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called );
                            } else {                         
                ...
                $this->save_called_credit_details_in_order( $order->id, null );
                        ...
                        function save_called_credit_in_session( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
                            ...
                            if ( !empty( $coupons ) && $this->is_coupon_amount_pick_from_product_price( $coupons ) && !( $_product->get_price() > 0 ) ) {
                                if ( function_exists( 'get_product' ) ) {
                                    if ( !isset( Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called ) ) {
                                        Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called = array();
                                    }
                                    Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called += array( $cart_item_key => $cart_item_data['credit_amount'] );
                                } else {
                        ...
                        function call_for_credit_cart_item_data( $cart_item_data = array(), $product_id = '', $variation_id = '' ) {
            ...
            function execute_smart_coupons_shortcode( $atts ) {
                ...
                $coupon_exists = $wpdb->get_var("SELECT ID
                                                    ...
                                                    WHERE posts.post_title LIKE 'strtolower( $coupon_code )'
            ...
            function smart_coupons_json_search( $x = '', $post_types = array( 'shop_coupon' ) ) {
                ...
                $all_discount_types = ( Smart_Coupons_WC_Compatibility::is_wc_21() ) ? wc_get_coupon_types() : $woocommerce->get_coupon_discount_types();
                        ...
                        function override_price_before_calculate_totals( $cart_object ) {
                            ...
                            foreach ( $cart_object->cart_contents as $key => $value ) {
                                ...
                                if ( !empty( $coupons ) && $this->is_coupon_amount_pick_from_product_price( $coupons ) && !( $value['data']->price > 0 ) ) {
                                    ...
                                    if( function_exists( 'get_product' ) ) {
                                        $price = ( isset( Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called[$key] ) ) ? Smart_Coupons_WC_Compatibility::global_wc()->session->credit_called[$key]: '';
                                    } else {                         
                                        $price = ( isset( $_SESSION['credit_called'][$key] ) ) ? $_SESSION['credit_called'][$key]: '';
                                    }

                                    if ( $price <= 0 ) {
                                        Smart_Coupons_WC_Compatibility::global_wc()->cart->set_quantity( $key, 0 );    // Remove product from cart if price is not found either in session or in product
                                        continue;
                                    }
                            ...
                            if ( !empty( $coupons ) && $this->is_coupon_amount_pick_from_product_price( $coupons ) && ( !( $product->get_price() != '' || ( is_plugin_active( 'woocommerce-name-your-price/woocommerce-name-your-price.php' ) && ( get_post_meta( $product->id, '_nyp', true ) == 'yes' ) ) ) ) ) {
                                ...
                                <div id="call_for_credit">
                                    ...
                                    <?php 
                                        ...
                                        echo '</p><br /><br />';
                                        echo "<input id='credit_called' step='any' type='number' min='1' name='credit_called' value='' autocomplete='off' autofocus />";
                                    ?>
                ...
                if ( $this->show_available_coupons() ) {
                    ...
                    $js = " jQuery(function(){
                            // Apply Coupon through Ajax
                                ...
                                jQuery.ajax({
                                    url:    '". esc_url( Smart_Coupons_WC_Compatibility::global_wc()->cart->get_cart_url() ) ."',
                                    type:   'post',
                                    data:   {
            ...
            function show_available_coupons_before_checkout_form() {
                ...
                if ( $this->show_available_coupons() ) {
                    global $woocommerce;
                    $param_name = ( Smart_Coupons_WC_Compatibility::is_wc_21() ) ? 'wc_checkout_params' : 'woocommerce_params';
                    $js = " jQuery(function(){
                                ...
                                jQuery('div.apply_coupons_credits').on('click', function() {
                                    ...
                                    jQuery.ajax({
                                         type:      'POST',
                                         url:       " . $param_name . ".ajax_url,
                                         dataType:  'html',
                                         data: {
                                            action:             'woocommerce_apply_coupon',
                                            security:           " . $param_name . ".apply_coupon_nonce,
                                            coupon_code:        coupon_code
                                         },
            ...
            function show_available_coupons() {
                ...
                if ( empty( $coupons ) ) return false;
                    ...
                    <div id='coupons_list'><h2><?php _e( stripslashes( $smart_coupon_cart_page_text ), 'wc_smart_coupons' ) ?></h2>
                        ...
                        // NEWLY ADDED CODE TO MAKE COMPATIBLE.
                        if( function_exists( 'get_product' ) ){
                            $coupons_applied = Smart_Coupons_WC_Compatibility::global_wc()->cart->get_applied_coupons();
                        } else {
            ...
            function gift_certificate_receiver_detail_form() {
                ...
                foreach ( Smart_Coupons_WC_Compatibility::global_wc()->cart->cart_contents as $product ) {
            ...
            function smart_coupons_admin_settings( $wc_general_settings = array() ) {
            ...
            function apply_smart_coupon_to_cart() {
                ...
                Smart_Coupons_WC_Compatibility::global_wc()->cart->smart_coupon_credit_used = array();

                if (Smart_Coupons_WC_Compatibility::global_wc()->cart->applied_coupons) {

                    foreach (Smart_Coupons_WC_Compatibility::global_wc()->cart->applied_coupons as $code) {
                        ...
                        if ( $smart_coupon->is_valid() && $smart_coupon->type=='smart_coupon' ) {
                            ...
                            if ( Smart_Coupons_WC_Compatibility::global_wc()->cart->discount_total != 0 && ( Smart_Coupons_WC_Compatibility::global_wc()->cart->discount_total + $smart_coupon->amount ) > $order_total ) {
                                $smart_coupon->amount = $order_total - Smart_Coupons_WC_Compatibility::global_wc()->cart->discount_total;
                            } elseif( $smart_coupon->amount > $order_total ) {
                                $smart_coupon->amount = $order_total;
                            }

                            Smart_Coupons_WC_Compatibility::global_wc()->cart->discount_total      = Smart_Coupons_WC_Compatibility::global_wc()->cart->discount_total + $smart_coupon->amount;
                            Smart_Coupons_WC_Compatibility::global_wc()->cart->smart_coupon_credit_used[$code]     = $smart_coupon->amount;
                        }
                    }
                }
            }
                ...
                if( Smart_Coupons_WC_Compatibility::global_wc()->cart->applied_coupons ) {
                    ...
                    foreach( Smart_Coupons_WC_Compatibility::global_wc()->cart->applied_coupons as $code ) {
                        ...
                        if($smart_coupon->type == 'smart_coupon' ) {
                            ...
                            update_post_meta( $order_id, 'smart_coupons_contribution', Smart_Coupons_WC_Compatibility::global_wc()->cart->smart_coupon_credit_used );
            ...
            function sc_json_search_coupons( $x = '', $post_types = array( 'shop_coupon' ) ) {
                global $woocommerce, $wpdb;
            ...
            function woocommerce_product_options_coupons() {
                ...
                <?php
                    ...
                    <script type="text/javascript">
                        ...
                        var updateSendCouponOnRenewals = function() {
							...
							<?php if( Smart_Coupons_WC_Compatibility::is_wc_21() ) { ?>
                ...
                <img class="help_tip" data-tip='<?php _e('These coupon/s will be given to customers who buy this product. The coupon code will be automatically sent to their email address on purchase.', 'wc_smart_coupons'); ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" width="16" height="16"/></p>
            ...
            function woocommerce_gift_certificates_email_template_path( $template ) {

                $template_name  = 'email.php';
                $template_path  = 'woocommerce-smart-coupons/';
            ...
            function get_global_coupons() {
                ...
                return $coupons;
                        ...
                        //
                        function send_gift_certificate( $email, $amount, $message = '' ){
                                ...
                                <form action="<?php echo admin_url('admin.php?page=woocommerce_coupon_csv_import&action=sent_gift_certificate'); ?>" method="post">
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" min="0.01" step="any" name="smart_coupon_amount" id="amount" placeholder="<?php _e('0.00', 'wc_smart_coupons'); ?>" class="input-text" />
                                            </td>
                                            <td>
                                                <?php
                        ...
                        function admin_generate_bulk_coupons_and_export(){
                            ...
                            $assets_path    = str_replace( array( 'http:', 'https:' ), '', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() ) . '/assets/';

                            if ( Smart_Coupons_WC_Compatibility::is_wc_21() ) {

                                // Register scripts
                                wp_register_script( 'woocommerce_admin', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/admin/woocommerce_admin' . $suffix . '.js', array( 'jquery', 'jquery-blockui', 'jquery-ui-sortable', 'jquery-ui-widget', 'jquery-ui-core', 'jquery-tiptip' ), Smart_Coupons_WC_Compatibility::global_wc()->version );
                                wp_register_script( 'woocommerce_admin_meta_boxes', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/admin/meta-boxes' . $suffix . '.js', array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'accounting', 'round' ), Smart_Coupons_WC_Compatibility::global_wc()->version );
                                // wp_register_script( 'woocommerce_admin_meta_boxes_variations', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/admin/meta-boxes-variations' . $suffix . '.js', array( 'jquery', 'jquery-ui-sortable' ), Smart_Coupons_WC_Compatibility::global_wc()->version );
                                wp_register_script( 'ajax-chosen', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/chosen/ajax-chosen.jquery' . $suffix . '.js', array('jquery', 'chosen'), Smart_Coupons_WC_Compatibility::global_wc()->version );
                                wp_register_script( 'chosen', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/chosen/chosen.jquery' . $suffix . '.js', array('jquery'), Smart_Coupons_WC_Compatibility::global_wc()->version );
                                ...
                                // wp_enqueue_script( 'chosen-rtl', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/chosen/chosen-rtl' . $suffix . '.js', array( 'jquery' ), Smart_Coupons_WC_Compatibility::global_wc()->version, true );
                                wp_enqueue_style( 'woocommerce_admin_styles', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/css/admin.css' );
                                wp_enqueue_style( 'woocommerce_chosen_styles', $assets_path . 'css/chosen.css' );
                                
                            } else {
                                // Register scripts
                                wp_register_script( 'woocommerce_admin', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/admin/woocommerce_admin' . $suffix . '.js', array ('jquery', 'jquery-ui-widget', 'jquery-ui-core' ), '1.0' );
                                wp_register_script( 'woocommerce_writepanel', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/admin/write-panels' . $suffix . '.js', array ('jquery' ) );
                                wp_register_script( 'ajax-chosen', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/js/chosen/ajax-chosen.jquery' . $suffix . '.js', array ('jquery' ), '1.0' );
                                ...
                                wp_enqueue_style( 'woocommerce_chosen_styles', Smart_Coupons_WC_Compatibility::global_wc()->plugin_url() . '/assets/css/chosen.css' );
                                    ...
                                    <div class="panel woocommerce_options_panel">
                                        <div class="option_group">
                                            ...
                                            <p class="form-field">
                                                ...
                                                <?php
                                        $coupon_types = ( Smart_Coupons_WC_Compatibility::is_wc_21() ) ? wc_get_coupon_types() : $woocommerce->get_coupon_discount_types();
                                            ...
                                            <p class="form-field">
                                                ...
                                                <select id="product_ids" name="product_ids[]" class="ajax_chosen_select_products_and_variations" multiple="multiple" data-placeholder="<?php _e( 'Search for a product&hellip;', 'wc_smart_coupons' ); ?>">
                                                    ...
                                                    ?>
                                                </select> <img class="help_tip" data-tip='<?php _e( 'Products which need to be in the cart to use this coupon or, for "Product Discounts", which products are discounted.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
                                            </p>
                                            ...
                                            <p class="form-field">
                                                ...
                                                <select id="exclude_product_ids" name="exclude_product_ids[]" class="ajax_chosen_select_products_and_variations" multiple="multiple" data-placeholder="<?php _e( 'Search for a product…', 'wc_smart_coupons' ); ?>">
                                                    ...
                                                    ?>
                                                </select> <img class="help_tip" data-tip='<?php _e( 'Products which must not be in the cart to use this coupon or, for "Product Discounts", which products are not discounted.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
                                            </p>
                                            ...
                                            <p class="form-field">
                                                ...
                                                <select id="product_categories" name="product_categories[]" class="chosen_select" multiple="multiple" data-placeholder="<?php _e( 'Any category', 'wc_smart_coupons' ); ?>">
                                                    ...
                                                    ?>
                                                </select> <img class="help_tip" data-tip='<?php _e( 'A product must be in this category for the coupon to remain valid or, for "Product Discounts", products in these categories will be discounted.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
                                            </p>
                                                ...
                                                <select id="exclude_product_categories" name="exclude_product_categories[]" class="chosen_select" multiple="multiple" data-placeholder="<?php _e( 'No categories', 'wc_smart_coupons' ); ?>">
                                                    ...
                                                    if( isset( $post ) )                                            
                                                        ...
                                                        if ( $categories ) foreach ( $categories as $cat )
                                                            echo '<option value="' . esc_attr( $cat->term_id ) . '"' . selected( in_array( $cat->term_id, $category_ids ), true, false ) . '>' . esc_html( $cat->name ) . '</option>';
                                                    ?>
                                                </select> <img class="help_tip" data-tip='<?php _e( 'Product must not be in this category for the coupon to remain valid or, for "Product Discounts", products in these categories will not be discounted.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
                                            </p>
                                            ...
                                            <p class="form-field">
                                                ...
                                                <img class="help_tip" data-tip='<?php _e( 'List of emails to check against the customer&#39;s billing email when an order is placed. Enter comma (,) separated e-mail ids. One coupon code will be assigned to one e-mail. Number of e-mails should be equal to number of coupons to be generated.', 'wc_smart_coupons' ) ?>' src="<?php echo Smart_Coupons_WC_Compatibility::global_wc()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
                                            </p>
                ...
                <?php  
                    ...
                    background-image:url("<?php echo plugins_url( 'assets/images/import.png' , __FILE__ ) ; ?>");
                        ...
                        //
                        function generate_coupons_code( $post, $get, $post_ids, $coupon_postmeta_headers = array() ) {
                        ...
                        function wc_smart_coupons_export_headers( $coupon_postmeta_headers = array() ) {
                            
                            $sc_postmeta_headers = array( 'auto_generate_coupon'       => __( 'Auto Generate Coupon', 'wc_smart_coupons' ),
                                                        'coupon_title_prefix'          => __( 'Coupon Title Prefix', 'wc_smart_coupons' ),
                                                        'coupon_title_suffix'          => __( 'Coupon Title Suffix', 'wc_smart_coupons' ),
                                                        'sc_coupon_validity'           => __( 'SC Coupon Validity', 'wc_smart_coupons' ),
                                                        'validity_suffix'              => __( 'Validity Suffix', 'wc_smart_coupons' ),
                                                        'sc_is_visible_storewide'      => __( 'SC Is Visible Storewide', 'wc_smart_coupons' ),
                                                        'sc_disable_email_restriction' => __( 'SC Disable Email Restriction', 'wc_smart_coupons' ),
...
//                                      Create CSV file
                                    ...
                                    <script type="text/javascript">
                                        jQuery(function(){
                                            ...
                                            if ( jQuery('select#discount_type').val() == 'smart_coupon' ) {
                                                ...
                                                jQuery('#minimum_amount').parent('p').hide();
                                                jQuery('#individual_use').parent('p').hide();
                                                jQuery('#exclude_sale_items').parent('p').hide();
                                                jQuery('#product_ids').parent('p').hide();
                                                jQuery('#exclude_product_ids').parent('p').hide();
                                                jQuery('#product_categories').parent('p').hide();
                                                jQuery('#exclude_product_categories').parent('p').hide();
                                                jQuery('#usage_limit').parent('p').hide();
                                                jQuery('#usage_limit_per_user').parent('p').hide();
                                            } else {
                                            ...
                                            jQuery('#minimum_amount').parent('p').show();
                                            jQuery('#individual_use').parent('p').show();
                                            jQuery('#exclude_sale_items').parent('p').show();
                                            jQuery('#product_ids').parent('p').show();
                                            jQuery('#exclude_product_ids').parent('p').show();
                                            jQuery('#product_categories').parent('p').show();
                                            jQuery('#exclude_product_categories').parent('p').show();
                                            jQuery('#usage_limit').parent('p').show();
                                            jQuery('#usage_limit_per_user').parent('p').show();
                                                }
                                            };
                        ...
                        function sc_woocommerce_coupon_options_usage_restriction() {
