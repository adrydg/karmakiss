...
class WC_Points_Rewards_Points_Log {
	...
	public static function query( $args ) {
		...
		$found_rows = '';
		if ( isset( $args['calc_found_rows'] ) && $args['calc_found_rows'] ) $found_rows = 'SQL_CALC_FOUND_ROWS';

		// distinct results?
		$distinct = '';
		if ( isset( $args['distinct'] ) && $args['distinct'] ) $distinct = 'DISTINCT';

		// returned fields
		$fields = "{$wc_points_rewards->user_points_log_db_tablename}.*";
		if ( ! empty( $args['fields'] ) && is_array( $args['fields'] ) ) $fields .= ', ' . implode( ', ', $args['fields'] );

		// joins
		$join = '';
		if ( ! empty( $args['join'] ) && is_array( $args['join'] ) ) $join = implode( ' ', $args['join'] );

		// where clauses
		$where = '';
		if ( ! empty( $args['where'] ) ) $where = "AND " . implode( " AND ", $args['where'] );

		// group by
		$groupby = '';
		if ( ! empty( $args['groupby'] ) && is_array( $args['groupby'] ) ) $groupby = implode( ', ', $args['groupby'] );
		...
		if ( ! empty( $args['per_page'] ) && empty( $args['paged'] ) ) $args['paged'] = 1;
		...
		if ( isset( $args['calc_found_rows'] ) && $args['calc_found_rows'] ) self::$found_rows = $wpdb->get_var( 'SELECT FOUND_ROWS()' );
