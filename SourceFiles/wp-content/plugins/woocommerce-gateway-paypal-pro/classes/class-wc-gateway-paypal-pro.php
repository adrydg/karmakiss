...
class WC_Gateway_PayPal_Pro extends WC_Payment_Gateway {
	...
	function process_payment( $order_id ) {
		...
		if ( $this->enable_3dsecure ) {
		    ...
		    $woocommerce->session->set( 'Centinel_TermUrl', str_replace('http:', 'https:', add_query_arg('wc-api', 'WC_Gateway_PayPal_Pro', home_url('/'))) );
	...
	function authorise_3dsecure() {
		...
		$this->log->add( 'paypal-pro', 'authorise_3dsecure() for order ' . absint( $order_id ) );
