		...
		if ( is_array( $product_addons ) && ! empty( $product_addons ) ) {
			...
			foreach ( $product_addons as $addon ) {
				...
				if ( is_wp_error( $data ) ) {
					...
					$cart_item_meta['addons'] = array_merge( $cart_item_meta['addons'], apply_filters( 'woocommerce_product_addon_reorder_cart_item_data', $data, $addon, $product_id, $post_data ) );
				}
			}
		}
