...
class CF_Connection
{
    ...
    function get_container($container_name=NULL)
    {
        ...
        #}
        if ($status == 404) {
			return 'Error: Container not found. You must create it first. Status: `' . $status . '`. Reason: `' . $reason . '`.';
        }
        if ($status < 200 || $status > 299) {
			return 'Error: Invalid container response. Status: `' . $status . '`. Reason: `' . $reason . '`. Message: `' . $this->cfs_http->get_error() . '`.';
        }
        return new CF_Container($this->cfs_auth, $this->cfs_http, $container_name, $count, $bytes);
    }
...
class CF_Object
{
    ...
    function _guess_content_type($handle) {
        ...
        // Still no content type so emulate in PHP. - Dustin Aug 1, 2012.
        if (!$this->content_type && (string)is_file($handle) ) {
        	$this->content_type = $this->my_mime_content_type( $handle );
        }
		
		// Still no content type so just use a default. - Dustin Aug 1, 2012.
		if (!$this->content_type) {
